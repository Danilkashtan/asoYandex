...
<form action="${path}" method="post">
        ...
        <label class = "col-sm-2 col-form-label"> User Name :</label>
            <div class="col-sm-6">
                <input type="text" name="username" value="<#if user??>${user.username}</#if>"
                       class = "form-control form-control ${(usernameError??)?string('is-invalid', '')}" placeholder="User name"/>
                <#if usernameError??>
    ...
    <div class = "col-sm-6">
        <div class="g-recaptcha" data-sitekey="6Lc926kUAAAAAFtmdrThYMY8QZ7BgVE4U96PMB43"></div>
        <#if captchaError??>
    ...
    <button class = "btn btn-primary " style="margin-left:90px;" type="submit"><#if !isRegisterForm> Sign In<#else>Sign up</#if></button>
</form>
</#macro>
